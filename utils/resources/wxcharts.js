    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        points.forEach(function (item, index) {
            if (item !== null) {
                ...
                context.beginPath();
                context.setFillStyle(eachSeries.setColor[index]);
                if(eachSeries.isGradation){
                    var grad = context.createLinearGradient(startX, item.y, startX, height + item.y);
                    grad.addColorStop(0, eachSeries.setColor[index].start);
                    grad.addColorStop(1, eachSeries.setColor[index].end);
...
function drawXAxis(categories, opts, config, context) {
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        if(opts.subCategories && opts.subCategories.length > 0){
            context.beginPath();
            context.setFillStyle(opts.subCategoriesColor || '#666666');
            opts.subCategories.forEach(function (item, index) {
                var offset = eachSpacing / 2 - measureText(item) / 2;
                context.fillText(item, xAxisPoints[index] + offset, startY + config.fontSize + 18);
            });
            context.closePath();
            context.stroke();
        }
...
function drawCharts(type, opts, config, context) {
    ...
    var duration = opts.animation ? 300 : 0;
